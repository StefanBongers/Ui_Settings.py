...
* Mit den Pfeiltasten kann man zwischen den Feldern hin- und herspringen. Die Nutzung der TAB-Taste (vorwärts) und Shift+TAB (rückwärts) ist ebenfalls zu empfehlen.
* Es gibt einige Shortcuts:

    * Strg+B ==> Schnellauswahl für die Beringerauswahl
    * Strg+S ==> Speichern
    * Strg+F ==> Schaltet Brutfleck ein/aus
    * Strg+N ==> Neue Eingabe
    * Esc    ==> Beendet Bearbeitung
    * Strg+W ==> Schließt Eingabemaske
    * Strg+Q ==> Beendet das Programm
    * Alt +E ==> öffnet Eingabemaske

* Die Zeichen / * - und + auf dem Nummernfeld (rechte Tastaturseite) sind analog für U M N und J. So kann ab Fett alles über das Numemrnfeld eingegeben werden (probiert es aus!)

Beim Aufruf über das Menü wartet Geier auf die Eingabe einer Art. Der *deutsche Artname* wird erwartet. Die Art kann auch über das Dropdown ausgewählt werden.
...
* **f** für Fremdfang (wenn sicher ist, dass es sich um einen Wiederfang handelt, der nicht an der eigenen Station und Zentrale beringt wurde - auch dies schlägt das Programm selbstständig vor, falls der Ring nicht in der Datenbank enthalten ist)
